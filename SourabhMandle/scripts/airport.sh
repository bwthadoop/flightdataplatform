...
# Description    :Airport data migration bash                                     #
...
# shellcheck disable=SC2154
mysql -u${username} -p${password} -e "insert into ${audit_dataset_name}.${audit_table_name}(job_id,job_name,run_date,run_status) values(${job_id},'${job_name1}',current_date(),'RUNNING')" #>>$log
...
#exec &> >(tee -a "$log")
# shellcheck disable=SC2154
sqoop import --connect "jdbc:mysql://${hostname}:${port}/${database_name}" --username "${username}" --password password --table $1 --target-dir "${target_dir_path1}"/"$1" --delete-target-dir --split-by "$2"
...
echo "INFO:successfully imporetd sqoop job" #>>$log
...
mysql -u${username} -p${password} -e "update ${audit_dataset_name}.${audit_table_name} set run_status='COMPLETED', last_job_value='${max_update_datetime}' where job_id=${job_id} and job_name='${job_name1}'" #>>$log
...
hive -e"
          ...
          LOAD DATA INPATH '${target_dir_path1}/$1/*' OVERWRITE INTO TABLE bwt_airportdata_arc.airport;



          "
if [ $? -ne 0 ]; then
    echo "Error:failed to load data into hive table : bwt_airportdata_arc" #>>$log
    exit 1
fi

echo "INFO:Successfully loaded the data into hive table : bwt_airportdata_arc" #>>$log
