...
#log data
echo "Student_Data_Migration" >>"${logfile_location}"
...
if [ $? -ne 0 ]
then
echo ${log_date_time} ${bashfile_name} "ERROR: failed to import credential_config.config file" >>"${logfile_location}"
exit 1
fi
echo ${log_date_time} ${bashfile_name}"INFO: successfully import credential_config.config file" >>"${logfile_location}"

#check job config file
. job.config
if [ $? -ne 0 ]
then
echo  ${log_date_time} ${bashfile_name}"ERROR: failed to import job_config.config file" >>"${logfile_location}"
exit 1
fi
echo ${log_date_time} ${bashfile_name}"INFO: successfully import job_config.config file" >>"${logfile_location}"
